...
class OnDemandCaptureSessionWrapper(
...
) : CaptureSessionWrapper(unityListener, frameCallback, width, height, 2) {
    ...
    private val dummySurfaceTexture = SurfaceTexture(0).apply {
        setDefaultBufferSize(1, 1)
    }

    /** Dummy surface for preview capture request. */
    private val dummySurface = Surface(dummySurfaceTexture)

    /**
     * Creates a new capture session and sets the repeating preview request.
     */
     ...
     */
    fun takePicture(captureTemplate: Int): Boolean {
    ...
    /**
     * Same as [CaptureSessionWrapper.closeSession], but also releases [dummySurfaceTexture].
     */
// In OnDemandCaptureSessionWrapper.kt
    override fun closeSession() {  // Jetzt kann die Methode überschrieben werden
